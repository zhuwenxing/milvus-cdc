...
def milvus_recall_test(host='127.0.0.1', port=19530, index_type="HNSW"):
    logger.info(f"recall test for index type {index_type}")
    file_path = f"{str(Path(__file__).absolute().parent)}/assets/ann_hdf5/sift-128-euclidean.hdf5"
...
if __name__ == "__main__":
    ...
    for index_type in ["HNSW"]:
        milvus_recall_test(host, port, index_type)
